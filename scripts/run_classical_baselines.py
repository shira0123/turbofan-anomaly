...
def _metric_rows(
    *,
    ...
    rows: list[dict] = []
    selection_pr_auc: list[float] = []
    selection_roc_auc: list[float] = []
    for policy_frame in policy_frames:
        ...
        selection_policy = bool(policy_frame["selection_policy"].iloc[0])
        if selection_policy and np.isfinite(pr_auc) and np.isfinite(roc_auc):
            selection_pr_auc.append(pr_auc)
            selection_roc_auc.append(roc_auc)
    ...
    if len(selection_pr_auc) != 3:
        raise RuntimeError("All three registered endpoint selection policies are required")
    return rows, float(np.mean(selection_pr_auc)), float(np.mean(selection_roc_auc))
