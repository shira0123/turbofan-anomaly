...
from src.models.classical_baselines import EmpiricalCDFCalibrator
...
def _require_hash(path: Path, expected: str) -> None:
    ...
    if observed != expected:
        raise RuntimeError(
            f"Registered input hash mismatch for {path}: "
...
def _selection_metrics(
    ...
    for policy_frame in policy_frames:
        ...
        scores = calibrated_scores[included]
        ...
        metric_rows.append(
            {
                ...
                "policy_semantics": str(
                    policy_frame["policy_semantics"].iloc[0]
                ),
    ...
    if len(selection_pr) != 3:
        raise RuntimeError("Exactly three endpoint selection policies are required")
    return metric_rows, float(np.mean(selection_pr)), float(np.mean(selection_roc))
...
def main() -> None:
    ...
    _require_hash(Path(inputs["split_manifest"]), inputs["split_manifest_sha256"])
    _require_hash(
        Path(inputs["preprocessing_selection"]),
        inputs["preprocessing_selection_sha256"],
    )
    _require_hash(
        Path(inputs["baseline_config"]), inputs["baseline_config_sha256"]
    )
    metadata_dir = Path(inputs["window_metadata_dir"])
    sequence_dir = Path(inputs["sequences_dir"])
    ...
    result_path = Path(protocol["result_config_output"])
