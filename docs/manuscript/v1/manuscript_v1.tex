...
\item Can a frozen reconstruction-based policy keep the proxy-healthy endpoint false-positive rate at or below 6\% across the three registered onset proxies?
...
\input{tables/table_04_literature_comparison.tex}
...
\input{tables/table_01_dataset_protocol_summary.tex}
...
For clarity, \textbf{P0} denotes the global-scaling control and \textbf{P1/K=6} the selected six-context preprocessing pipeline. A \textbf{mode} is a learned operating-context cluster, an \textbf{endpoint} is the final cycle of a 30-cycle window, and \textbf{proxy-healthy} refers only to endpoints preceding a registered normalized-life onset. \textbf{Internal held-out} denotes the untouched 52-engine partition drawn from the FD002 training trajectories; it is distinct from NASA's separately supplied official test trajectories.
...
\input{tables/table_02_frozen_primary_policy.tex}
...
\input{tables/table_03_confirmatory_results.tex}
