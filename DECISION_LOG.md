...
## 2026-08-27 — Recommend the governed Phase 5 PCA alert policy at Gate 4

- **Metadata-lineage correction:** Preserve the original Phase 1 window metadata unchanged and derive P1/K=6 window context from the registered cycle frame at each window endpoint: `window.op_mode = cycle_frame.op_mode[(engine, window.end_cycle)]`. Blocked protocol v1 remains immutable; protocol v2 records both source hashes and both derived hashes. No mode was reclustered, inferred from sensors, or assigned by majority vote.
- **Frozen study retained:** Five score sources, two threshold contexts, eight threshold rules, four EWMA states, four persistence values, five proxy policies, the `<=60/1,000` healthy-endpoint FAR feasibility constraint, event definitions, and lexicographic selection order were unchanged. The study produced 280 threshold rows, 1,280 candidates, and 6,400 candidate-policy rows.
- **Overall validation recommendation:** `pca_reconstruction__per_mode__quantile_0.995__ewma_alpha_0.20__persistence_8`. Across the three primary endpoint policies, its minimum/mean engine detection coverage was `57.69%/78.21%`, worst false-positive endpoint rate `44.711/1,000`, worst false-alert-event rate `3.514/1,000`, worst healthy-false-alert engine rate `44.23%`, median-of-policy median delay `23` cycles, and median-of-policy median lead `18` cycles.
- **Target attainment:** The endpoint final-10%/20%/30% FAR values were `44.711`, `15.342`, and `9.053` per 1,000, so all met the `<6%` target. Median delays were `7`, `23`, and `38` cycles; only final-10% met the `<=12`-cycle aspiration, and the aggregate 23-cycle delay did not.
- **Negative results retained:** The registered per-detector objective selected conservative LOF and Isolation Forest policies with zero false alerts but also zero detected engines. One-Class SVM and the LSTM ensemble were feasible but had lower minimum coverage than PCA; the LSTM had higher mean coverage and a lower worst FAR, but the predeclared selection objective prioritizes minimum coverage first.
- **Sensitivity:** Full-window final-20% excluded 1,505 ambiguous windows and produced precision/recall/F1 `0.8435/0.8594/0.8514`; full-window final-30% excluded 1,508 and produced `0.9473/0.5544/0.6995`.
- **Complementarity boundary:** Pairwise detection/miss and healthy-false-alert overlap was recorded for later consideration. No score or decision fusion was implemented or evaluated.
- **Verification:** Five registered score sources, 5,037 training rows per source, 9,365 validation rows per source, 280 thresholds, 1,280 candidates, 6,400 candidate-policy rows, 16 report hashes, five local bundle hashes, and six ledger rows passed independent verification. The full repository suite passed 103 tests; all 10 script and 10 console help checks passed.
- **Evidence boundary:** This is validation-proxy operational evidence, not physical onset accuracy or held-out performance. No internal-test or official NASA-test data was accessed, and no online recalibration or fusion ran.
